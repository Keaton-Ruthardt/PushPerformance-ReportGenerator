...
/**
 * Hop Test Comparative Analysis Service
 * Compares individual athlete Hop Test metrics against professional athlete population
 */
...
/**
 * Get professional athlete bucket statistics for all Hop Test metrics
 * Returns mean, std dev, and percentile distribution for each metric
 */
async function getProAthleteStats() {
  try {
    console.log('📊 Fetching pro athlete Hop Test statistics from BigQuery...');
    console.log('📊 HOP_METRICS object:', HOP_METRICS);

    // Build dynamic SQL to get stats for all metrics
    // Use p1 and p99 instead of MIN/MAX to filter outliers
    const metricStats = Object.entries(HOP_METRICS).map(([key, column]) => `
      -- ${key}
    ...
    console.log(`✅ Loaded stats from ${stats.total_tests} pro athlete Hop Test tests`);
 ...
 * @param {string} metricKey - The metric key (e.g., 'rsi', 'jumpHeight', 'gct')
 * @param {object} proStats - Pro athlete statistics object
 * @param {boolean} lowerIsBetter - For GCT, lower values are better
...
function calculatePercentile(value, metricKey, proStats, lowerIsBetter = false, convertToInches = false) {
  ...
  // For GCT, invert the percentile (lower is better)
  if (lowerIsBetter) {
    ...
    // Normal percentile (higher is better) - for RSI and Jump Height
...
/**
 * Get comparative analysis for an athlete's Hop Test results
...
async function getComparativeAnalysis(athleteMetrics) {
  try {
    ...
    for (const [key, column] of Object.entries(HOP_METRICS)) {
...
/**
 * Get all Hop Test tests for a specific athlete from BigQuery
 */
async function getAthleteHistory(profileId, limit = 10) {
  try {
    const metricColumns = Object.values(HOP_METRICS).join(', ');
